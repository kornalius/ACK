...
const userPath = path.join(app.getPath('home'), '/.shapy')
...
const mouseEvent = function (e) {
  return {
    ...
    x: e.clientX,
    y: e.clientY,
    localX: e.clientX - e.currentTarget.offsetLeft,
    localY: e.clientY - e.currentTarget.offsetTop,
    clientX: e.clientX,
    clientY: e.clientY,
    pageX: e.pageX,
    pageY: e.pageY,
...
const $ = function (selector) {
  ...
  let query = function (el, selector) {
    for (let e of el.children) {
      if (e.matches(selector)) {
        return e
      }
      if (e._shadowRoot) {
        let r = query(e._shadowRoot, selector)
        if (r) {
          return r
        }
      }
      let r = query(e, selector)
      if (r) {
        return r
      }
    }
  ...
  return query(document.body, selector)
}

const offsetLeft = function (el, parent) {
  let l = 0
  while (el && el !== parent) {
    l += el.offsetLeft
    el = el.offsetParent
...
const offsetTop = function (el, parent) {
  let t = 0
  while (el && el !== parent) {
    t += el.offsetTop
    el = el.offsetParent
...
const disableSelectables = function () {
  _.each(selectables(), e => {
    e._userSelect = e.style.userSelect
    e.style.userSelect = 'none'
  })
}

const enableSelectables = function () {
  _.each(selectables(), e => {
    e.style.userSelect = e._userSelect
  })
}

const observeMutation = function (el, cb, options = { attributes: false, childList: false, subtree: false, characterData: false, attributeOldValue: false, characterDataOldValue: false, attributeFilter: undefined }) {
  let mo = new window.MutationObserver(entries => _.each(entries, cb))
  mo.observe(el, options)
  return function () { mo.disconnect(el) }
}

const observeResize = function (el, cb) {
  let ro = new window.ResizeObserver(entries => _.each(entries, cb))
  ro.observe(el)
  return function () { ro.unobserve(el) }
}

const observeIntersection = function (el, cb, options = { root: undefined, rootMargin: undefined, threshold: undefined }) {
  let io = new IntersectionObserver(entries => _.each(entries, cb), options)
  io.observe(el)
  return function () { io.unobserve(el) }
}
